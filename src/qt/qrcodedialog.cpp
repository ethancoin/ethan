...
{
    QString ret = QString("Ethan:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ETHAN unit input in lnReqAmount, we generate the URI with ETHAN as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(EthanUnits::format(EthanUnits::ETHAN, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
